...
export const  getPageSearch = async (key: string | null, pageParam = 1): Promise<InfinityPage> => {
    if (key === null) {
        return {
    ...
    const cards: CardProps[] = json.data.map(page => cardBuilder(page.attributes))
    return {
        nextCursor: pagination.page + 1 <= pagination.pageCount ? pagination.page + 1 : undefined,
